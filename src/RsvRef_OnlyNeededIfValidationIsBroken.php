<?php
$flash_error = $_SESSION['customer_error'] ?? '';
$flash_success = $_SESSION['customer_success'] ?? '';
unset($_SESSION['customer_error'], $_SESSION['customer_success']);
...
//---------------------------
    ...
    if (empty($errors))
    {
        ...
        catch(PDOException $e)
        {
            ...
            $_SESSION['customer_error'] = "Error creating reservation: " . esc($e->getMessage());
            header("Location: " . $_SERVER['PHP_SELF']);
            exit;
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete'])) 
{
    $errors = []; 
    $success = "";
    $reservation_id = clean('reservation_id');
    ...
    if (isset($_POST['update'])) 
    {
        ...
        if(empty($errors))
        {
          try
          {
            ...
            $_SESSION['customer_success'] = "Reservation $id updated successfully.";
            header("Location: " . $_SERVER['PHP_SELF']);
            exit;
          }
          catch(Exception $e)
          {
            ...
            $_SESSION['customer_error'] = "Could not update reservation: ". esc($e->getMessage());
            header("Location: " . $_SERVER['PHP_SELF']);
            exit;
          }
        }
...
<head>
  ...
  <style>
    /* Optional: purely presentational helpers (safe to remove) */
    .form-table { width: 100%; border-collapse: collapse; }
    .form-table th, .form-table td { padding: .6rem .8rem; vertical-align: middle; }
    .form-table th { text-align: left; }
    .section-header th { background: #f3f3f3; font-size: 1rem; }
    .actions-row td { padding-top: 1rem; }
    .inline-table { width: auto; border-collapse: collapse; }
    .inline-table th, .inline-table td { padding: .4rem .6rem; }
  </style>
</head>
<body>
  ...
  <!-- Create Reservation (table layout) -->
  <form action="" method="post" id="createReservation" class="reservation-form">
    <table class="form-table" aria-describedby="create-caption">
      <caption id="create-caption" style="text-align:left; padding:.5rem 0; font-weight:bold;">Create Reservation</caption>
      <colgroup>
        <col style="width: 22%;">
        <col>
      </colgroup>

      <tr class="section-header"><th colspan="2">Customer</th></tr>
      <tr>
        <th scope="row"><label for="firstName">First Name</label></th>
        <td><input id="firstName" type="text" name="firstName" value="<?php echo esc($firstName); ?>"></td>
      </tr>
      <tr>
        <th scope="row"><label for="lastName">Last Name</label></th>
        <td><input id="lastName" type="text" name="lastName" value="<?php echo esc($lastName); ?>"></td>
      </tr>
      <tr>
        <th scope="row"><label for="phone">Phone Number</label></th>
        <td><input id="phone" type="text" name="phone" value="<?php echo esc($phone); ?>"></td>
      </tr>
      <tr>
        <th scope="row"><label for="email">Email</label></th>
        <td><input id="email" type="text" name="email" value="<?php echo esc($email); ?>"></td>
      </tr>

      <tr class="section-header"><th colspan="2">Address</th></tr>
      <tr>
        <th scope="row"><label for="address">Street Address</label></th>
        <td><input id="address" type="text" name="address" value="<?php echo esc($address); ?>"></td>
      </tr>
      <tr>
        <th scope="row"><label for="city">City</label></th>
        <td><input id="city" type="text" name="city" value="<?php echo esc($city); ?>"></td>
      </tr>
      <tr>
        <th scope="row"><label for="zip">Zipcode</label></th>
        <td><input id="zip" type="text" name="zip" value="<?php echo esc($zip); ?>"></td>
      </tr>

      <tr class="section-header"><th colspan="2">Rental Details</th></tr>
      <tr>
      ...
      <tr>
        ...
        <td><input id="quantity" type="number" name="quantity" value="<?php echo esc($quantity); ?>"></td>
      </tr>

      <tr class="section-header"><th colspan="2">Dates</th></tr>
      <tr>
        <th scope="row"><label for="startDate">Start Date</label></th>
        <td><input id="startDate" type="date" name="startDate" value="<?= esc($startDate) ?>"></td>
      </tr>
      <tr>
        <th scope="row"><label for="endDate">End Date</label></th>
        <td><input id="endDate" type="date" name="endDate" value="<?= esc($endDate) ?>"></td>
      </tr>
  ...
  <!-- Admin: Delete a Pending Reservation -->
  <section class="admin-delete">
    <h3>Admin: Delete a Pending Reservation</h3>
    <form action="" method="post" onsubmit="return confirm('This permanently deletes the reservation and its items. Continue?');">
      <table class="inline-table" aria-label="Delete reservation">
        <tr>
          <th scope="row"><label for="reservation_id_delete">Reservation ID</label></th>
          <td>
            <input id="reservation_id_delete" type="number" name="reservation_id" min="1" required>
  ...
  <!-- Search and Update Reservations -->
  <section class="admin-search">
    <h3>Search and Update Reservations</h3>
    <form method="post" id="searchForm">
      <table class="inline-table" aria-label="Search reservations">
        <tr>
          <th scope="row"><label for="search_field">Field</label></th>
          <td>
            <select name="field" id="search_field" onchange="this.form.submit()">
              <option value="">Select...</option>
        ...
        <?php
          // Dynamic input type based on selected field
        ...
        ?>
        <tr>
          <th scope="row"><label for="search_value">Value</label></th>
          <td>
            <input id="search_value" type="<?= $input_type ?>" name="search_value" value="<?= esc($input_value) ?>">
  ...
  <!-- Display Results / Inline Update (unchanged except for surrounding structure) -->
  <?php if ($show_results): ?>
...
<?php foreach ($reservations as $res): 
  $fid = 'f'.(int)$res['reservation_id']; ?>
  <tr>
    <!-- one empty form per row -->
    <form id="<?= $fid ?>" method="post"></form>

    <td>
      <?= esc($res['reservation_id']) ?>
      <input type="hidden" name="reservation_id" value="<?= esc($res['reservation_id']) ?>" form="<?= $fid ?>">
    </td>

    <td><input type="text"   name="phone"      value="<?= esc($res['phone']) ?>"        form="<?= $fid ?>"></td>
    <td><input type="text"   name="address"    value="<?= esc($res['address']) ?>"      form="<?= $fid ?>"></td>
    <td><input type="text"   name="city"       value="<?= esc($res['city']) ?>"         form="<?= $fid ?>"></td>
    <td><input type="text"   name="zip"        value="<?= esc($res['zip']) ?>"          form="<?= $fid ?>"></td>
    <td><input type="date"   name="start_date" value="<?= esc($res['start_date']) ?>"   form="<?= $fid ?>"></td>
    <td><input type="date"   name="end_date"   value="<?= esc($res['end_date']) ?>"     form="<?= $fid ?>"></td>

    <!-- status as a safe select (prevents typos) -->
    <td>
      <select name="order_status" form="<?= $fid ?>">
    ...
    <td>
      <button type="submit" name="update" form="<?= $fid ?>">Save</button>
    </td>
  </tr>
<?php endforeach; ?>
</tbody>
